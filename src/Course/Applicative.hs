...
lift4 ::
  ...
  -> k d
  -> k e
lift4 f ka kb kc = (pure f <*> ka <*> kb <*> kc <*>)
...
filtering ::
  ...
  -> List a
  -> k (List a)
filtering f = foldRight (\x xs -> lift2 (\b l -> if b then x:.l else l) (f x) xs) (pure Nil) 
-----------------------
-- SUPPORT LIBRARIES --
-----------------------
